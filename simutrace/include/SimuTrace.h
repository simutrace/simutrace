...
#define SIMUTRACE_API
     ...
     *
     *  \returns \c TRUE if successful, \c FALSE otherwise. For a more detailed
     ...
     */
    SIMUTRACE_API
    BOOL StSessionClose(SessionId session);


    /*! \brief Creates or opens a store.
     *
     ...
     *  store. Opening a store is thus typically the next operation after
     *  creating a new session. This method either creates a new stores or
     *  overwrites or opens an existing one.
     *
     ...
     *                      it already exists. Supply \c FALSE to open an
     *                      existing store. This will fail if the store cannot
     *                      be found. Supply \c TRUE to always create a new 
     *                      store, potentially overwriting an existing one.
     *
     *  \returns \c TRUE if successful, \c FALSE otherwise. For a more detailed
     *           error description call StGetLastError().
     *
     ...
     *           \c TRUE will fail if the store is still open in another 
     *           session.
     *
     ...
     *  to close a store without creating a new session.
     *
     ...
     *  can be used with StGetNextEntryFast() to read entries.
     *
     *  \param session The id of the session that holds the stream of interest.
     *                 Must be \c INVALID_SESSION_ID, when supplying
     *                 \p handle.
     ...
     *           more detailed error description call StGetLastError(). If 
     *           successful, the handle will \b NOT point to the exact entry 
     *           requested by the <\c type, \c value>-pair. The method only 
     *           guarantees that the requested entry is in the data delivered
     *           by the server for this request. To find the desired entry,
     *           use StGetNextEntryFast() on the returned handle and scan over
     *           the entries manually. This may change in the future.
     *
     *  \remarks A stream may have an arbitrary number of open stream handles.
     *
     *  \remarks To not attempt to write data with a handle received from this
     ...
     *           invalidated. It may be used to retry the operation. If 
     *           successful, the caller must cast the pointer to the data type 
     *           (e.g., #DataWrite64) for the type supplied at stream 
     *           registration.
     *
    ...
    static inline void* StGetNextEntryFast(StreamHandle* handlePtr)
    {
        ...
        byte* nextEntry = entry + handle->entrySize;
        ...
        if (nextEntry > handle->segmentEnd) {
            ...
            *handlePtr = handle;
            ...
            entry     = handle->entry;
            nextEntry = entry + handle->entrySize;
        }

        assert((entry >= handle->segmentStart) && 
               (entry < handle->segmentEnd));

        handle->entry = nextEntry;
        return entry;
    }
