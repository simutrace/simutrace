...
static const uint64_t memoryEntriesCount = (64 MiB * 10) / sizeof(DataWrite64) + 100;
...
int memoryWriterThreadMain(Thread<SessionId>& thread)
{
    ...
    _bool success = StSessionOpen(session);
    if (!success) {
        printLastError();
        return -1;
    }
...
int memoryReaderThreadMain(SessionId session, StreamId stream)
{
    ...
    try {
        ...
        // end of the stream and read backwards (just for the fun of it)
        handle = StStreamOpen(session, stream,
                              QueryIndexType::QIndex, 0,
                              StreamAccessFlags::SafSequentialScan |
                              StreamAccessFlags::SafReverseQuery |
                              StreamAccessFlags::SafReverseRead,
                              nullptr);

        ThrowOnNull(handle, Exception);

        uint64_t i = memoryEntriesCount;
        while (true) {
            i--;

            // (1) Get a pointer to the next entry
            DataWrite64* entry = reinterpret_cast<DataWrite64*>(
                StGetPreviousEntryFast(&handle));
            if (entry == nullptr) {
                ExceptionInformation info;
                StGetLastError(&info);
...
int main(int argc, char *argv[])
{
    ...
    try {
        ...
        do {
            // This time, we open the store, which will open the existing store
            // and not create a new one. Since the server might still be busy
            // running down the store due to our previous session close, we
            // retry the operation if necessary.
            success = StSessionOpenStore(session, "simtrace:mystore.sim");
        } while (shouldRetry());
        ThrowOn(!success, Exception);
